...
**Weeks 1–3 of the 4-week plan are built**: auth (magic link), campaigns (create / join by invite code), sessions (DM recaps in markdown), quotes (any member), entities (NPC/location/faction cards with the secret→reveal mechanic), a `log_entries` timeline, and per-entity private notes. `requirements.md` is the source of truth for scope and the data model. The complete schema + RLS is applied to the Supabase project (`rijdpyfrbhjehtrkoeka`) and version-controlled in `supabase/migrations/` (`0002`–`0013`). The reveal/hide mechanic is the `reveal_entity`/`hide_entity` RPCs (flip visibility + write/hide a `reveal` timeline moment atomically). Next per the plan: Week 4 (polish, empty states, responsive pass, demo data, real playtest).
